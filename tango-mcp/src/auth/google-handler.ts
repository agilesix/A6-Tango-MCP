...
app.get("/authorize", async (c) => {
	...
	if (!c.env.COOKIE_ENCRYPTION_KEY || !c.env.OAUTH_KV) {
		return c.text("OAuth not configured - missing COOKIE_ENCRYPTION_KEY or OAUTH_KV", 500);
	}
...
app.get("/callback", async (c) => {
	...
	if (!c.env.OAUTH_KV || !c.env.GOOGLE_CLIENT_ID || !c.env.GOOGLE_CLIENT_SECRET) {
		return c.text("OAuth not configured - missing required environment variables", 500);
	}
