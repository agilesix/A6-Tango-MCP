...
import path2__default, { dirname, join } from 'path';
...
function configureCommandForJson(cmd) {
  cmd.configureOutput({
    writeErr: (str) => {
  ...
  cmd.exitOverride((err) => {
    ...
    process.exit(1 /* VALIDATION_ERROR */);
  });
  cmd.commands.forEach((subCmd) => configureCommandForJson(subCmd));
}
function configureCommandForText(cmd) {
  cmd.showHelpAfterError("\n\u{1F4A1} Tip: Run with --help to see all available commands and options");
  cmd.configureOutput({
    writeErr: (str) => {
...
function createProgram() {
  ...
  program.name("mcp-server-kit").description(`Extensible scaffolding tool and test harness for MCP servers
...
AI Agent Tips:
  ...
  \u2022 Check --help on any command for detailed options`).version(VERSION, "-v, --version", "Output the current version");
  program.addCommand(createNewCommand());
  program.addCommand(createTemplateCommand());
  program.addCommand(createAddCommand());
  program.addCommand(createValidateCommand2());
  program.addCommand(createListCommand2());
  if (jsonMode) {
...
function run() {
  ...
  if (process.argv.length <= 2) {
    console.log("\u{1F44B} Welcome to mcp-server-kit!\n");
